...
> the reason with the new online simulator is good; but say that we actually want to make it nearly serverless to host it only reliable on github pages.
> the possibility would also be there, since there exist a CIB (https://github.com/tbfleming/cib) so we could compile the generated C++ code to webassembly and run it directly 
> in the browser. But CIB does not support dynamic linking yet; and therefore, we cannot link against our large Armadillo mathematics library; and static compilation would
> extend the linking process to several minutes.
> Therefore, we decided to use a stateless server application: the user sends its C&C files to server and the server translates it to WebAssembly; and then the client runs
> the simulation.
> This has the advantage the server is not needed during the simulation process, and the server must not track the state (whether the user pauses the simulation and so on),
> and we do not run into critical performance issues (when the simulation does not work fluently this has massive impact on the user experience; when the server is used by multiple
> users the compilation process will take longer; this is not nice, but you still can see the fluent driving car)
> Also the maintenance of a stateless server is much simpler as the maintenance of stateful server; as stateless tests (and later their derived performance tests) are much simpler.
